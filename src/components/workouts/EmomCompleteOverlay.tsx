...
import type { EmomWorkoutResult } from '@/types/customWorkouts';
import type { DailyWorkoutBonus } from '@/types/titles';
import { useTheme } from '@/hooks/use-theme';

interface EmomCompleteOverlayProps {
  result: EmomWorkoutResult;
  dailyBonus?: DailyWorkoutBonus | null;
}

export function EmomCompleteOverlay({ result, dailyBonus = null }: EmomCompleteOverlayProps) {
    ...
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <View style={styles.statsRow}>
        <View style={[styles.statCard, { backgroundColor: theme.backgroundElement, borderColor: theme.border }]}>
          <Text style={[styles.statValue, { color: theme.primary }]}>{result.completedIntervals}</Text>
          <Text style={[styles.statLabel, { color: theme.textSecondary }]}>Minutes hit</Text>
        </View>
        <View style={[styles.statCard, { backgroundColor: theme.backgroundElement, borderColor: theme.border }]}>
          <Text style={[styles.statValue, { color: theme.xp }]}>{result.totalReps}</Text>
        ...
        {result.exerciseBreakdown.map((entry) => (
          <View key={entry.exerciseType} style={styles.breakdownRow}>
            ...
            <Text style={[styles.breakdownMeta, { color: theme.textSecondary }]}>
              {entry.targetReps} per minute · {entry.totalReps} total
            </Text>
          </View>
        ))}
      </View>
      ...
      <Text style={[styles.duration, { color: theme.textSecondary }]}>
        {formatWorkoutTimeLimit(result.timeLimitSeconds)} session
      </Text>
    </View>
  );
}
