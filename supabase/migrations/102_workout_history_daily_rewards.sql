-- Expose daily first-workout XP/coins on activity history for solo workout runs.
-- Coin amount matches award_daily_workout_completion base (see dailyWorkoutRewards.ts).
...
begin
  ...
  with combined as (
    ...
    select
      ...
      case
        when daily_xp.amount is not null then v_daily_workout_coins
        else null
      end as coins_earned
