-- Friends-only XP leaderboard (weekly + all-time), scoped to accepted friends + current user.
...
begin
  ...
  if p_period = 'weekly' then
    ...
    weekly_totals as (
      ...
      join friend_ids fi on fi.user_id = e.user_id
  ...
  ranked as (
    ...
    join friend_ids fi on fi.user_id = p.id
    left join public.user_equipped_items uei_avatar
      on uei_avatar.user_id = p.id and uei_avatar.slot = 'avatar'
    left join public.shop_items avatar_item on avatar_item.id = uei_avatar.item_id
    left join public.user_equipped_items uei_frame
...
grant execute on function public.get_friends_xp_leaderboard(text, integer) to authenticated;
